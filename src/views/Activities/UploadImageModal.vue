...
const props = defineProps(['subCatId'])
...
async function submitImages(){
    ...
    if (hasEmptyImage) {
        ...
        // imageFields.map(data => {
        //     dataNoErrorField.push({
        //         subcategory: {
        //             connect: [{id: props.subCatId}]
        //         },
        //         image: data.image
        ...
        // await postData('api/subcategory-images',dataNoErrorField).then((result) => {
        ...
        for (const data of imageFields) {
            const entryData = {
                subcategory: { id: props.subCatId },
                image: data.image,
            };

            await postData('api/subcategory-images', entryData).then((result) => {
                if (!result.status == 200) {
                throw new Error(response.status)
            }
                Swal.fire('Success',
