...
const getActivityData = computed(() => {
  return storeActivities.activityData;
});
...
async function assignDataToForm(){
    if(isEdit.value){
        await storeActivities.fetchCategoryData(route.params.id);
        formData.sub_category_name = getActivityData.value.sub_category_name
        formData.sub_category_description = getActivityData.value.sub_category_description
        formData.sub_category_img_url = getActivityData.value.sub_category_img_url
    }
}
...
<template>
    <Home>
        ...
        <p class="text-2xl font-semibold pb-6 text-gray-700">   {{  isEdit ? 'Update' : 'Add' }} Activity</p>
        <Form class="w-full" @submit="submitActivity" :validation-schema="schema" v-slot="{ errors }">
                <!--body-->
