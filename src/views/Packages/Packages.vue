...
const getPackages = computed(() => {
  return storePackages.packages;
});

function toogleVisibility(event, id){
  
  Swal.fire({
     title: `Are you sure you want to ${event.target.checked ? 'enable' : 'disable'} this package?`,
     showDenyButton: true,
...
<template>
     <Home>
        <div>
            <p class="text-2xl font-semibold pb-6 text-gray-700">Package Management</p>
            <button 
                @click="addPackage"
        ...
        <div class="relative overflow-x-auto sm:rounded-lg border border-gray-200">
            ...
            <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400 table-fixed">
                ...
                <tbody>
                    ...
                    v-for="packageName in getPackages" :key="packageName">
                    <td scope="row" class="px-6 py-4 font-medium whitespace-nowrap">
                            {{ packageName.category.category_name}}
                        </td>
                        <td scope="row" class="px-6 py-4 font-medium whitespace-nowrap">
                            {{ packageName.sub_category_name }}
                        </td>
                        <td class="px-6 py-4">
                            {{ packageName.sub_category_description }}
                        </td>
                        <td class="px-6 py-4">
                            <img
                            :src="packageName.sub_category_img_url"
                            class="h-auto w-full"
                            alt="Error Image" />
                        </td>
                        <td class="px-6 py-4">
                            <label :for="`toggleVisibility-${packageName.id}`"  class="flex items-center cursor-pointer">
                            <!-- toggle -->
                            <div class="relative">
                              <!-- input -->
                              <input type="checkbox" 
                                :id="`toggleVisibility-${packageName.id}`" 
                                class="sr-only" 
                                @change="toogleVisibility($event, packageName.id)"
                                :checked="packageName.isEnabled" />
                              <!-- line -->
                        ...
                        <td class="px-6 py-4">
                          <button 
                            @click="router.push(`/packages/images/${packageName.id}`)"
                          ...
                          <button 
                            @click="router.push(`/package/itinerary/${packageName.id}`)"
                            ...
                            <br><br>
                            <button 
                                @click="router.push(`/packages/edit/${packageName.id}`)"
                            ...
                            <button 
                            @click="deletePackage(packageName.id)"
