...
async function getImages(){
    ...
    await fetchData(`/api/transfer-images/${route.params.vehicleId}`).then(res =>{
        images.value = res
    }).catch(err => {})
...
function deleteImage(imageId){
    Swal.fire({
      ...
      if (result.isConfirmed) {
        deleteData(`api/transfer-images/${imageId}`).then((result) => {
...
onMounted(() => {
  storeTransfers.fetchTransferData(route.params.vehicleId);
  getImages()
...
<template>
    <Home>
        <div>
            ...
            <p class="text-2xl font-semibold pb-6 text-gray-700">Add Images for {{ getTransferData.vehicle_type }}</p>
            <button 
            @click="addImageModal()"
            ...
            <button type="button" 
                class="sm:text-xs xs:text-xs md:text-base text-white bg-slate-700 hover:bg-slate-600 mt-5 focus:ring-4 focus:outline-none font-medium rounded-lg text-md px-7 py-3"
                @click="router.push('/transfers-and-rentals')">
                    ...
                    <div class="w-full p-1 md:p-2">
                        <div
                            class="block bg-white shadow-[0_2px_15px_-3px_rgba(0,0,0,0.07),0_10px_20px_-2px_rgba(0,0,0,0.04)]">
                                <img
                                class="w-full md:h-80 sm:h-full object-cover"
                                :key="i"
                                :src="image.image"
                                alt="" />
                            <div class="p-4">
        ...
        <div class="fixed z-10 overflow-y-auto top-0 w-full left-0" id="modal" v-if="isModalVisible">
          <div class="flex items-center justify-center min-height-100vh pt-4 px-4 pb-20 text-center sm:block sm:p-0">
            ...
            <div class="inline-block align-center bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle w-1/2" role="dialog" aria-modal="true" aria-labelledby="modal-headline">
              ...
              <hr>
              <image-upload-modal :vehicleId="route.params.vehicleId" @closeModal="closeModal"></image-upload-modal>
            </div>
        </div>
        </div>
